                ...
                async for line in response.aiter_lines():
                    ...
                    if chunk:
                        ...
                        is_technical = (
                            ...
                            stripped.startswith('<functions') or
                            stripped.startswith('</functions') or
                            stripped.startswith('{') or
                            stripped.startswith('}') or
                            stripped == '' or
                            (len(stripped) < 20 and ':' in stripped)  # Short chunks with colons are likely JSON
                        )
            ...
            if final_content:
                ...
                if extracted:
                    ...
                    is_only_tool_call = (
                        ...
                        (content_stripped.startswith('{') and '"name"' in content_stripped and '"arguments"' in content_stripped)
                    )
                    ...
                    if is_only_tool_call:
                        ...
                        if text_before:
                            text_before = re.sub(r'</?response>', '', text_before, flags=re.IGNORECASE).strip()
