...
from src.agentframework.config import (
    find_config_path,
    load_config,
    get_safety_config,
    get_tools,
...
def test_get_tools_instantiation():
    ...
    with patch(
        "src.agentframework.config.TOOL_REGISTRY", {"read_file": MagicMock()}
    ) as mock_registry:
...
def test_get_tools_default_params():
    ...
    with (
        patch("src.agentframework.config.TOOL_REGISTRY", {"cat_tool": mock_tool_class}),
        patch(
            "src.agentframework.config.TOOL_CONFIG_KEYS",
            {"cat_tool": {"param": "default"}},
        ),
    ):
