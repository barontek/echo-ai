...
from src.agentframework.router import SemanticRouter, RouteSelection
...
async def test_semantic_router_selection():
    ...
    # Test Code Routing
    agent.llm.extract_structured.return_value = RouteSelection(reasoning="It is a code request", selected_agent="code_agent")
    result1 = await router.route("Write a python script to sort an array")
    assert result1 == "code_agent"

    # Test Research Routing
    agent.llm.extract_structured.return_value = RouteSelection(reasoning="It is a research prompt", selected_agent="research_agent")
    result2 = await router.route("Who won the 1993 world series?")
    assert result2 == "research_agent"

    # Test Fallback Routing
    agent.llm.extract_structured.return_value = RouteSelection(reasoning="General question", selected_agent="default")
