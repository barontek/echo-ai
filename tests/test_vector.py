...
class TestVectorStoreAddDocuments:
    ...
    async def test_add_documents_with_ids(self, store):
    ...
    async def test_add_documents_auto_generates_ids(self, store):
        ids = await store.add_documents(["doc1"])
        assert len(ids) == 1
        assert isinstance(ids[0], str)

    async def test_add_documents_with_metadatas(self, store):
        _ids = await store.add_documents(
            ["doc1"],
    ...
    async def test_add_documents_default_metadata(self, store):
        _ = await store.add_documents(["doc1"])
        store.collection.add.assert_called_once()
        call_kwargs = store.collection.add.call_args.kwargs
        assert call_kwargs["metadatas"] == [{"source": "manual_ingestion"}]

    async def test_add_documents_filters_invalid_metadata(self, store):
        _ = await store.add_documents(
            ["doc1"],
    ...
    async def test_add_documents_no_collection(self, store):
...
class TestVectorStoreSearch:
    ...
    async def test_search_returns_structured_results(self, store):
    ...
    async def test_search_no_metadatas_fallback(self, store):
        store.collection.query.return_value = {
            "documents": [["doc1"]],
            "distances": [[0.5]],
        }
    ...
    async def test_search_no_collection(self, store):
        store.collection = None
        with pytest.raises(RuntimeError, match="not initialized"):
            await store.search("test")

    async def test_search_passes_n_results(self, store):
        store.collection.query.return_value = {"documents": [[]]}
        await store.search("query", n_results=10)
